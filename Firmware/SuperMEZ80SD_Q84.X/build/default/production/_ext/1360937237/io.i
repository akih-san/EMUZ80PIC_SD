...
void io_handle() {
    ...
    if ((board_rd_pin_hook?(*board_rd_pin_hook)():1) && (board_wr_pin_hook?(*board_wr_pin_hook)():1) && 0 <= io_interrupt_data) {


        io_stat_ = IO_STAT_INTR_WAITING;
        (*board_set_data_dir_hook)(0x00);
        (*board_set_data_pins_hook)(io_interrupt_data & 0xff);
        io_interrupt_data = -1;
        goto let_cpu_read_data;
    }

    if ((board_rd_pin_hook?(*board_rd_pin_hook)():1)) {
 ...
 enter_bus_master:
    ...
    case 170:
        ...
        if (invoke_monitor) {
            ...
            int fetch_count = 5;
            while (0 < fetch_count) {
                if (rd == 0 && (board_rd_pin_hook?(*board_rd_pin_hook)():1) == 1) {

                    fetch_count--;
                }
                rd = (board_rd_pin_hook?(*board_rd_pin_hook)():1);
                (*board_clock_op_hook)(-5);
            }


            (*board_set_busrq_pin_hook)(0);
            (*board_clock_op_hook)(-1);
...
int io_wait_write(uint8_t wait_io_addr, uint8_t *result_io_data)
{
    ...
    while (1) {
        ...
        if ((board_rd_pin_hook?(*board_rd_pin_hook)():1) == 0 && io_addr == 0x4e) {
# 734 "../src/io.c"
            (*board_set_busrq_pin_hook)(0);
            (*board_set_data_dir_hook)(0x00);
            (*board_set_data_pins_hook)(disk_stat);
        ...
        if ((board_rd_pin_hook?(*board_rd_pin_hook)():1) == 0 || (io_addr != 0x41 && io_addr != wait_io_addr)) {

            printf("%s: ERROR: I/O %5s %3d, %3d (%02XH, %02XH) while waiting for %d (%02XH)\n\r",
                   __func__, (board_rd_pin_hook?(*board_rd_pin_hook)():1) == 0 ? "read" : "write",
...
0x31,0x00,0x08,0x3E,0x00,0xD3,0xAD,0x76,0x18,0xF6
# 786 "../src/io.c" 2
 };

    ((void)((io_stat() == IO_STAT_NOT_STARTED || io_stat() == IO_STAT_STOPPED || io_stat() == IO_STAT_MONITOR) || (__assert_fail("io_stat() == IO_STAT_NOT_STARTED || io_stat() == IO_STAT_STOPPED || io_stat() == IO_STAT_MONITOR", "../src/io.c", 789, __func__),0)));
    ...
    *saved_status = io_stat();
    ...
    io_wait_write((board_set_nmi_pin_hook != ((void*)0)) ? 174 : 171, ((void*)0));




    if ((board_set_nmi_pin_hook != ((void*)0)))
        mon_prepare_nmi();
    else
        mon_prepare();
    io_wait_write((board_set_nmi_pin_hook != ((void*)0)) ? 175 : 172, ((void*)0));
                         ...
                         const mem_region_t *outparams, unsigned int noutparams, int bank)
{
    ...
    ((void)((io_stat() == IO_STAT_MONITOR) || (__assert_fail("io_stat() == IO_STAT_MONITOR", "../src/io.c", 842, __func__),0)));

    size = 0;
    for (i = 0; i < ninparams; i++) {
        size = ((size) > (inparams[i].offs + inparams[i].len) ? (size) : (inparams[i].offs + inparams[i].len));
    }
...
void io_invoke_target_cpu_teardown(int *saved_status)
{
    ...
    ((void)((io_stat() == IO_STAT_MONITOR) || (__assert_fail("io_stat() == IO_STAT_MONITOR", "../src/io.c", 867, __func__),0)));
