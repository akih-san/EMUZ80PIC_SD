...
0x3A,0x08,0x00,0x4F,0xAF,0x47,0xED,0xB0,0x3E,0x00,0x18,0xE4
# 122 "../src/memory.c" 2
};
...
static void write_to_sram_low_addr(uint32_t dest, const void *buf, unsigned int len)
{
    ...
    while (0 < len) {
        n = ((0x80) < (len) ? (0x80) : (len));
        __memcpy_on_target(addr, sizeof(dma_helper_z80), (uint8_t*)buf, n, bank);
        len -= n;
        addr += n;
        buf += n;
    }
...
static void read_from_sram_low_addr(uint32_t src, void *buf, unsigned int len)
{
    ...
    while (0 < len) {
        n = ((0x80) < (len) ? (0x80) : (len));
        __memcpy_on_target(sizeof(dma_helper_z80), addr, (uint8_t*)buf, n, bank);
        len -= n;
        addr += n;
        buf += n;
    }
...
void __write_sram_regions(const mem_region_t *regions, unsigned int n, int bank)
{
    ...
    if ((board_write_sram_regions_hook != ((void*)0))) {
        (*board_write_sram_regions_hook)(regions, n, bank);
        return;
    }

    (*board_set_data_dir_hook)(0x00);
    for (i = 0; i < n; i++) {
        (*board_setup_addrbus_hook)((((uint32_t)(bank) << 16) + (regions[i].offs)));
        (*board_write_to_sram_hook)(regions[i].offs, regions[i].addr, regions[i].len);
    }
}
...
void __read_sram_regions(const mem_region_t *regions, unsigned int n, int bank)
{
    ...
    if ((board_read_sram_regions_hook != ((void*)0))) {
        (*board_read_sram_regions_hook)(regions, n, bank);
        return;
    }
    ...
    for (i = 0; i < n; i++) {
        (*board_setup_addrbus_hook)((((uint32_t)(bank) << 16) + (regions[i].offs)));
        (*board_read_from_sram_hook)(regions[i].offs, regions[i].addr, regions[i].len);
    }
}
